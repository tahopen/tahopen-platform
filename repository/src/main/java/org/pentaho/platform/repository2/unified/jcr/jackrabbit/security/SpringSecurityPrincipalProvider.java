...
/**
 * A Jackrabbit {@code PrincipalProvider} that delegates to a Tahopen
 * {@link UserDetailsService}.
 * <p/>
 * <p>
 * A {@code java.security.Principal} represents a user. A
 * {@code java.security.acl.Group} represents a group. In
 * Spring Security, a group is called a role or authority or granted authority.
 * Arguments to the method {@link
 * #getPrincipal(String)} can either be a Principal or Group. In other words,
 * {@link #getPrincipal(String)}
 * might be called with an argument of a Spring Security granted authority. This
 * happens when access control entries
 * (ACEs) grant access to roles and the system needs to verify the role is
 * known.
 * </p>
 * <p/>
 * <p>
 * Jackrabbit assumes a unified space of all user and role names. The
 * PrincipalProvider is responsible for
 ...
 * this implementation can take advantage
 * of a Spring Security UserCache. If available, it will use said cache for role
 * membership lookups. Also note that the
 * removal of a role or user from the system will not be noticed by this
 * implementation. (A restart of Jackrabbit is
 * required.)
 * </p>
 * <p/>
 * <p>
 * There are users and roles that are never expected to be in any backing store.
 * By default, these are "everyone" (a
 ...
 * a result, a {@code
 * NoSuchPrincipalException} is never thrown. See the method for details.
 * </p>
 *
 * @author mlowery
 */
public class SpringSecurityPrincipalProvider implements PrincipalProvider {
  ...
  /**
   * flag indicating whether or not UserDetailsService is called during creation
   * of user's principal
   * 
   ...
   * <p/>
   * <p>
   * Attempts to load user using given {@code principalName} using a Tahopen
   * {@code UserDetailsService}. If it fails
   * to find user, it returns a {@link Group} which will be caught by
   * {@code SpringSecurityLoginModule}.
   * </p>
   */
  ...
  protected UserDetails internalGetUserDetails(final String username) {
    ...
    // optimization for when running in pre-authenticated mode (i.e. Spring Security
    // filters have setup holder with
    ...
    if (getUserDetailsService() != null) {
      try {
        ...
        // We will use the authorities from the Authentication object of
      ...
      if (user != null) {
        ...
        newUser = new User(user.getUsername(), password, user.isEnabled(), ACCOUNT_NON_EXPIRED, CREDS_NON_EXPIRED,
  ...
  private void initSkipUserVerification(final Properties prop) {
    ...
    if (prop != null && prop.containsKey(SKIP_USER_VERIFICATION_PROP_KEY)
      ...
      // reading property from the class initialization properties is useful for unit
      // testing
