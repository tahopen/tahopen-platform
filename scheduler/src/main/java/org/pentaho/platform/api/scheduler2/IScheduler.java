...
/**
 * An object that allows for the scheduling of IActions on the Tahopen platform
 * 
 * @author arodriguez
 */
public interface IScheduler {
   ...
   *                  the user defined name for the job
   * @param actionId
   *                  the bean Id of the action to be run as defined an a plugin's
   *                  plugin.xml
   * @param jobParams
   *                  the parameters to be passed to the action when it runs
   * @param trigger
   ...
   *                             the action to be run at the scheduled time(s)
   * @param jobParams
   *                             the parameters to be passed to the action when it
   *                             runs
   * @param trigger
   *                             the time(s) at which to run the action
   * @param outputStreamProvider
   *                             if the action being scheduled expects to write to
   *                             an output stream, at the time of action execution
   *                             the
   *                             this provider will be used to create the stream
   *                             that passed to the action.
   * @return the scheduled job
   * @throws SchedulerException
   *                            If the job could not be scheduled
   */
   ...
   *                             the user defined name for the job
   * @param actionId
   *                             the bean Id of the action to be run as defined an
  ...
  /**
   * Updates both the parameters and trigger to be used to execute an existing
   * scheduled action.
   * 
   * @param jobId
  ...
  /**
   * Prevents the specified job from running in the future. The job remains in the
   * list of scheduled jobs in a "paused"
   * state. If the job is currently running, the currently running job is allowed
   * to complete.
   * 
   * @param jobId
  ...
  /**
   * Gets the scheduler availability window for all subjects for whom a window has
   * been set
   * 
   ...
   * Any currently running jobs are allowed
   * to complete. Note that the "paused" state of individual jobs is not changed
   * by this call.
   */
  public void pause() throws SchedulerException;

  /**
   * Allows the scheduler to process scheduled jobs. Note that the "paused" state
   * of individual jobs is not changed by
   * this call.
   */
  public void start() throws SchedulerException;

  /**
   * Shuts the scheduler down so it will process no more jobs. The implementation
   * will decide if this means kill jobs in
   * progress or let them finish.
   */
  public void shutdown() throws SchedulerException;

  /**
   * Sets the minimum time that must elapse between runs of any jobs. For example
   * if set to "5" then a job may not be
   ...
   * 
   * @param subject
   *                the subject whose min interval is being requested return the
   *                minimum interval or null if no interval has
   *                been set
   */
   ...
   * 
   * @param filter
   *               the filter to use to determine which jobs to return. If null
   *               all scheduled jobs are return.
  ...
  /**
   * A default implementation which doesn't do anything and exists for the
